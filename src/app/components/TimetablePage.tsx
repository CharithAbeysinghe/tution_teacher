...
export function TimetablePage() {
  const { data: timetableDays, loading, error } = useApi<TimetableDay[]>("/api/timetable");
  const today = new Date().toLocaleDateString("en-US", { weekday: "long" });
  const byDay = new Map((timetableDays ?? []).map((d) => [d.dayOfWeek, d.sessions]));
  ...
  return (
    <div style={{ fontFamily: "var(--font-body)" }}>
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        {loading && (
          <p style={{ color: "var(--muted-foreground)", fontSize: "0.95rem" }}>Loading timetable…</p>
        )}
        {error && (
          <p style={{ color: "#e74c3c", fontSize: "0.95rem" }}>Could not load timetable: {error}</p>
        )}
        {!loading && !error && (
          <>
            {/* Legend */}
            ...
            <div className="hidden md:grid grid-cols-7 gap-3">
              {days.map((day, index) => {
                const daySessions = byDay.get(index + 1) ?? [];
                return (
                  <div key={day}>
                    <div
                        ...
                        daySessions.map((cls, i) => {
                          ...
                          return (
                            <div key={i} className="rounded-xl p-3" style={{ background: colors.bg }}>
                              ...
                              <div style={{ color: "rgba(255,255,255,0.6)", fontSize: "0.65rem", marginTop: "4px", fontFamily: "var(--font-mono)" }}>{fmtTime(cls.startTime)} – {fmtTime(cls.endTime)}</div>
              ...
              {days.map((day, index) => {
                ...
                return (
                  <div key={day}>
                    ...
                    {daySessions.length === 0 ? (
                      <p style={{ color: "var(--muted-foreground)", fontSize: "0.85rem", paddingLeft: "1rem" }}>No classes scheduled.</p>
                    ) : (
                      <div className="flex flex-col gap-2">
                        {daySessions.map((cls, i) => {
                          const colors = subjectColors[cls.subject] || { bg: "var(--primary)", text: "#ffffff" };
                          return (
                            <div key={i} className="flex items-center gap-4 p-4 rounded-xl" style={{ border: "1px solid var(--border)", background: "var(--card)" }}>
                              ...
                              <div>
                                ...
                                <div style={{ color: "var(--muted-foreground)", fontSize: "0.82rem", marginTop: "2px" }}>{fmtTime(cls.startTime)} – {fmtTime(cls.endTime)} · {cls.room}</div>
